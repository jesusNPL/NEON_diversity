...
demonPrediction <- function(fit, variable = "PD", covariable = "SD", dimension, 
  ...
  if(groupLevel == TRUE) 
  { 
    pp <- data.frame(predict(fit, robust = TRUE))
    ...
    pp <- data.frame(predict(fit, re_formula = ~ (1 | Site)), robust = TRUE)
  ...
  corFit <- brm(data = pp1, family = student,
  ...
  smape <- mape(observed = pp[, variable], 
  ...
  srmsle <- Metrics::rmsle(actual = pp[, variable], 
                       predicted = pp[, "Estimate"])
  
  ## MAE - mean absolute error
  smae <- Metrics::mae(actual = pp[, variable], 
                         predicted = pp[, "Estimate"])
  
  ## Bias
  sbias <- Metrics::bias(actual = pp[, variable], 
                         predicted = pp[, "Estimate"])
  ...
  plotRES <- data.frame(pp, Deviation = PREDdev, Difference = PREDdiff, 
